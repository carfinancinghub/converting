...
const BankerLoanHistory = () => {
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:5000/api/loans/banker/${role}1`);
        ...
        <ul style={styles.list}>
          {loans.map(loan => (
            <li key={loan._id} style={styles.listItem}>
              Car ID: {loan.carId} - Buyer: {loan.buyer} - Amount: ${loan.amount} - Status: {loan.status}
            </li>
          ))}
        </ul>
      )}
...
export default BankerLoanHistory;