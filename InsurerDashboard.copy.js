...
const InsurerDashboard = () => {
  ...
  useEffect(() => {
    const fetchPolicies = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:5000/api/policies/insurer/${role}1`);
        ...
        <ul style={styles.list}>
          {policies.map(policy => (
            <li key={policy._id} style={styles.listItem}>
              Car ID: {policy.carId} - Buyer: {policy.buyer} - Policy Amount: ${policy.amount} - Status: {policy.status}
            </li>
          ))}
        </ul>
      )}
